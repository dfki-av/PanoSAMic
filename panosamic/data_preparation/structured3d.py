"""
Preprocessing Script for the Structured-3D dataset
...
def create_parser() -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        "--dataset_root",
        required=True,
        help="Path to the Structured-3D dataset",
    )
    parser.add_argument(
        "-o",
        "--output_root",
...
def read_sample_names(
    dir: Path,
    scene: str,
    corrupted_samples: list[str],
) -> list[str]:
    scene_samples = []
    for f in (dir / scene / "2D_rendering").iterdir():
        if not f.is_dir():
            continue
        short_path = scene + "/2D_rendering/" + f.stem
        if any(short_path in item for item in corrupted_samples):
            continue
...
def process_sample(
    ...
    input_sizes = []
    path = scene_pano_dir / sample_name / "panorama/full" / "rgb_rawlight.png"
    image_rgb = Image.open(path.absolute(), "r").convert("RGB")
    input_sizes.append(image_rgb.size)

    path = scene_pano_dir / sample_name / "panorama/full" / "depth.png"
    depth = Image.open(path.absolute(), "r")
    input_sizes.append(depth.size)

    path = scene_pano_dir / sample_name / "panorama/full" / "normal.png"
    normals = Image.open(path.absolute(), "r").convert("RGB")
    input_sizes.append(normals.size)

    path = scene_pano_dir / sample_name / "panorama/full" / "semantic.png"
    ...
    if save_depth_stats:
        ...
        max_depth = int(round(np.iinfo(np.uint16).max))
        depth_hist, bin_edges = np.histogram(
            masked_depth, bins=max_depth, range=(0, max_depth)
        )
...
def main():
    ...
    scene_list = [d for d in dir_contents if "scene_" in d]
    ...
    with open(output_dir / "assets" / "broken_files.json", "r") as fp:
    ...
    if args.overwrite or not cache_file.exists():
        ...
        for scene in scene_list:
            scene_dict[scene] = read_sample_names(dataset_dir, scene, corrupted_samples)
    ...
    for scene, sample_list in scene_dict.items():
        ...
        for sample_name in tqdm(
            ...
            sample_depth_hist = process_sample(
                dataset_dir / scene / "2D_rendering",
                scene_output_dir,
                sample_name,
                scale,
                save_depth_stats,
