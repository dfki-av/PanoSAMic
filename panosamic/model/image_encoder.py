...
This file contains code modified from the Segment Anything Model (SAM)
Copyright (c) Meta Platforms, Inc. and affiliates.
All rights reserved

Original code licensed under the Apache License, Version 2.0.

Modifications Copyright 2026 Mahdi Chamseddine.

The modifications in this file are licensed under the Creative Commons
...
"""panosamic:
This class is further adapted from the implementation of segment_anything available at:
https://github.com/facebookresearch/segment-anything/blob/main/segment_anything/modeling/image_encoder.py
Changes: Type hints, branch output at global attention indices, and renamed Block to TransformerBlock"""
...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, list[torch.Tensor]]:
        ...
        for i, blk in enumerate(self.blocks):
            x = blk(x)
            # Memory intensive, TODO alternative?
            if i in self.global_attn_indexes:
                branch_results.append(x.permute(0, 3, 1, 2))

        x = self.neck(x.permute(0, 3, 1, 2))

        return x, branch_results
